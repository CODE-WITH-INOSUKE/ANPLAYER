...
async function handleWSMessage(ws, type, data) {
  switch(type) {
    ...
    case 'load':
      ...
      // Check if we have a DB record for this song
      ...
      const dbSong = existing || song;
      const audioUrl = dbSong.is_downloaded && dbSong.path
        ? dbSong.path
        : (data.url || await search.getAudioUrl(song.youtube_id));
...
async function playNext() {
  ...
  if (queue.length > 0) {
    ...
    if (song) {
      ...
      const audioUrl = song.is_downloaded && song.path ? song.path : await search.getAudioUrl(song.youtube_id);
      mpv.load(audioUrl, song);
      broadcast({ type: 'queueUpdated' });
    }
  }
...
async function start() {
  ...
  // Wire download progress to WebSocket broadcast
  download.emitter.on('progress', (data) => {
    broadcast({ type: 'downloadProgress', data });
  });
  ...
  app.get('/downloads', (req, res) => {
    ...
    res.render('downloads', { downloads, title: 'Downloads - ANPlayer' });
  });
